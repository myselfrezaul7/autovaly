...
export default function ComparisonBanner() {
  ...
  return (
    <section className="relative w-full py-16 md:py-24 overflow-hidden bg-background">
      <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: "repeating-linear-gradient(45deg, #1C1F26 0, #1C1F26 1px, transparent 0, transparent 50%)", backgroundSize: "20px 20px" }} />
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8">
          <div className="w-full lg:w-[45%] flex items-center justify-center gap-4 md:gap-8">
            ...
            <div className="font-heading text-5xl md:text-7xl font-extrabold text-white/10 italic">VS</div>
            <m.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="flex flex-col items-center gap-4">
          ...
          <m.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="w-full lg:w-[45%] flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            <Link href="#" className="inline-flex items-center gap-2 px-7 py-3 bg-white text-background rounded-md font-bold text-sm uppercase tracking-wide transition-all duration-300 hover:scale-[1.03] hover:bg-accent hover:text-white">See Full Comparison <span aria-hidden="true">→</span></Link>
          </m.div>
        </div>
      </div>
    </section>
