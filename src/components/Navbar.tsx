        ...
        <div className="container mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <button 
              ...
              className="px-2 py-1 flex items-center text-sm font-bold rounded-md hover:bg-accent/10 transition-colors"
            >
            ...
            <button
              ...
              className="lg:hidden p-2 flex flex-col gap-1.5 ml-2"
      ...
      <AnimatePresence>
        {isSearchOpen && (
          <m.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-background/95 backdrop-blur-xl flex flex-col items-center pt-[15vh] px-4"
          >
              ...
              <form onSubmit={handleSearchSubmit}>
                <div className="flex items-center border-b-2 border-border-custom focus-within:border-accent transition-colors pb-4">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-2xl md:text-3xl font-heading font-bold outline-none text-text-light placeholder:text-muted/50"
                    autoFocus
                  />
                  {isTyping && (
            ...
            <m.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed top-0 right-0 h-full w-[280px] bg-surface z-50 border-l border-border-custom p-8 lg:hidden flex flex-col">
              ...
              <div className="mt-8 flex flex-col gap-2">
                ...
                <div className="mt-6 flex items-center justify-between py-4 border-b border-border-custom">
                  ...
                  <div className="flex items-center gap-2 bg-background p-1 rounded-md">
                    <button 
                      ...
                      className={`px-3 py-1 text-sm rounded ${currency === 'EUR' ? 'bg-accent text-white' : 'text-text-muted hover:text-text-light'}`}
                    >EUR</button>
                    <button 
                      onClick={() => { toggleCurrency(); setIsMobileMenuOpen(false); }}
                      className={`px-3 py-1 text-sm rounded ${currency === 'USD' ? 'bg-accent text-white' : 'text-text-muted hover:text-text-light'}`}
                    >USD</button>
                  </div>
                </div>
                ...
                <div className="mt-2 flex items-center justify-between py-4 border-b border-border-custom">
                  ...
                  <button onClick={() => { toggleTheme(); setIsMobileMenuOpen(false); }} className="p-2 bg-background rounded-md text-text-light hover:text-accent">
