    ...
    <div className="container mx-auto px-4 md:px-6 py-12">
      {garage.length >= 2 && (
        <div className="mb-8 flex justify-end">
          <Link href={compareUrl} className="inline-flex items-center gap-2 px-6 py-3 bg-accent text-white font-bold text-sm uppercase tracking-wide rounded hover:bg-accent-dark transition-colors shadow-lg shadow-accent/20">
          ...
          {garage.map((vehicle) => (
            ...
            >
              <TiltCard className="flex flex-col h-full overflow-hidden group">
                <Link href={`/vehicles/${vehicle.slug}`} className="block h-40 w-full relative" style={{ background: `linear-gradient(135deg, ${vehicle.coverGradient.from}, ${vehicle.coverGradient.to})` }}>
                  {vehicle.coverImage && (
                ...
                <div className="p-4 flex-1 flex flex-col relative">
                  <button 
                    ...
                    className="absolute right-4 top-4 w-8 h-8 rounded-full bg-surface/80 backdrop-blur border border-border-custom flex items-center justify-center text-muted hover:text-accent hover:border-accent transition-colors z-10"
