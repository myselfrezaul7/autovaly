...
export default function Newsletter() {
  ...
  return (
    <section className="w-full relative overflow-hidden gradient-mesh">
      ...
      <div className="container mx-auto px-4 md:px-6 py-16 md:py-20 relative z-10">
        <m.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="flex flex-col items-center text-center max-w-3xl mx-auto text-white">
          ...
          {status === "success" ? (
            <div className="bg-white/20 backdrop-blur-md border border-white/30 rounded-md px-8 py-6 text-lg font-bold">🎉 You&apos;re in! Welcome to the fast lane.</div>
          ) : (
            <form action="https://formspree.io/f/YOUR_FORM_ID" method="POST" onSubmit={() => setTimeout(() => setStatus("success"), 500)} className="w-full flex flex-col sm:flex-row gap-3 max-w-lg mb-6">
              <input type="email" name="email" placeholder="Enter your email address" className="flex-1 bg-white/10 backdrop-blur-md border border-white/20 text-white placeholder:text-white/50 px-5 py-4 rounded-md outline-none focus:bg-white/20 focus:border-white/40 transition-all" required />
              <button type="submit" className="bg-white text-accent font-bold uppercase tracking-wide px-8 py-4 rounded-md hover:bg-gray-100 transition-colors whitespace-nowrap">Subscribe</button>
            </form>
          )}

          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-white/60 mt-4">
