      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <m.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <form onSubmit={handleSearch} className="w-full max-w-2xl bg-background border border-border-custom rounded-md flex flex-col sm:flex-row p-2 focus-within:border-accent transition-colors shadow-2xl shadow-black/40">
              <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search make, model, year..." className="flex-1 bg-transparent border-none outline-none px-4 py-3 text-text-light placeholder:text-muted" />
            ...
            <div className="flex items-center gap-4 mt-6 text-xs font-semibold text-muted uppercase tracking-widest">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setQuery("Macan EV"); }} className="hover:text-accent transition-colors">Macan EV</button>
                <button onClick={() => { setQuery("R1T"); }} className="hover:text-accent transition-colors">R1T</button>
              </div>
            </div>
          </div>
        </m.div>
