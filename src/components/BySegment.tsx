    ...
    <section className="py-12 md:py-16 bg-surface border-y border-border-custom" id="by-segment">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="relative before:absolute before:left-0 before:top-0 before:bottom-4 before:w-6 before:bg-gradient-to-r before:from-surface before:to-transparent before:z-10 before:pointer-events-none after:absolute after:right-0 after:top-0 after:bottom-4 after:w-6 after:bg-gradient-to-l after:from-surface after:to-transparent after:z-10 after:pointer-events-none">
          <div className="flex gap-2.5 overflow-x-auto pb-4 mb-6 md:mb-10 hide-scrollbar touch-pan-x" role="tablist" aria-label="Vehicle Segments">
            {tabs.map((tab, idx) => (
              <button
                key={tab}
                ref={(el) => {
                ...
                aria-controls="segment-tabpanel"
                tabIndex={activeTab === tab ? 0 : -1}
                onClick={() => setActiveTab(tab)}
                onKeyDown={(e) => handleKeyDown(e, idx)}
                className={clsx(
                  "relative flex-shrink-0 px-5 py-2.5 rounded-full text-sm font-semibold uppercase tracking-wider transition-colors duration-300 border cursor-pointer",
                  activeTab === tab ? "text-white border-transparent" : "bg-transparent border-border-custom text-text-light hover:border-accent hover:text-accent"
                )}
              >
                {activeTab === tab && (
        ...
        <AnimatePresence mode="wait">
          <m.div
            ...
            id="segment-tabpanel"
            role="tabpanel"
            aria-labelledby={`segment-tab-${tabs.indexOf(activeTab)}`}
