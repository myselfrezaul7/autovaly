    ...
    <section className="py-12 md:py-16 lg:py-20 bg-surface border-y border-border-custom" id="evs">
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between mb-8 md:mb-10 pb-4 border-b border-border-custom">
          <h2 className="font-heading text-2xl md:text-3xl uppercase tracking-wider pl-4 border-l-4 border-accent flex items-center gap-2"><span className="text-accent">⚡</span> EV Spotlight</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {evSpotlightItems.map((ev, index) => (
            <m.article key={ev.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} className="group bg-background border border-border-custom rounded-md overflow-hidden flex flex-col">
              <div className="relative w-full h-48 md:h-56 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-black/60 backdrop-blur-md text-white text-xs font-bold rounded-sm border border-white/10 uppercase tracking-wide">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#00B894]"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
                    {ev.range}
                  </span>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-black/60 backdrop-blur-md text-white text-xs font-bold rounded-sm border border-white/10 uppercase tracking-wide">
              ...
              <div className="p-5 md:p-6 flex flex-col flex-1">
                ...
                <div className="text-xs font-bold text-accent uppercase tracking-widest mt-auto opacity-0 translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">Read Review →</div>
              </div>
            </m.article>
          ))}
        </div>
